...
asked all day — hours, address, phone, areas, services, the FAQ — and hands
anything about money to the phone. Turn it on per client:

```ts
chatbot: {
  enabled: true,
  endpoint: '/chat.php',   // generated by deploy-ftp; empty = no model at all
  notes: ['Dostępność towaru ustala się telefonicznie.'],
  maxTurns: 8,
},
```

**Three layers, and the model is the last one.** Quick replies and the FAQ
are answered in the browser from `/chatbot.json` — a static file the build
writes from `site.config.ts`. A typed question is checked against intent
patterns and the FAQ first; anything about prices, costs or stock gets the
hand-off line before any request is made. Only what is left goes to
`chat.php`, which builds its system prompt from the **same** `chatbot.json`,
so the model can never know something the page does not say. A retainer edit
to the config updates the assistant on the next deploy.

**What `chat.php` enforces**, whatever the model is told: the `Origin` must
be the site's own host; at most 8 turns of 500 characters; 6 calls a minute
and 40 a day per visitor; a daily cap for the whole site (`CHATBOT_DAILY_LIMIT`,
default 400) after which the widget hands off to the phone. The key lives in
`.chat-secret.php` one directory above the web root, written by the deploy
from `CHATBOT_API_KEY`. Usage (tokens, not text) goes to a monthly log in the
system temp directory, which is how you know what a client's assistant costs.

**Cost.** The default model is Claude Haiku 4.5 at $1 per million input
tokens and $5 per million output. The measured system prompt for Activa is
under 1,200 characters; a model call runs roughly 800 tokens in and 60 out —
well under a tenth of a US cent. A hundred conversations a month, with the
layers above catching most questions first, is cents. Change the model with
`CHATBOT_MODEL`.

**Performance.** The launcher is one button inside the layout chunk (7.5 KB
in total). The panel and the matcher are a separate 9 KB chunk, and the
knowledge file a 4 KB fetch, both made on the first click and never before.
Measured on Activa's homepage, mobile: 92 with the assistant on a quiet run
against 98 without it — inside the run-to-run spread seen on this build, but
not nothing. Every other route stayed at 97–99.
