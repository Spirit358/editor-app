...
export async function readBuild(outDir) {
  ...
  if (!postsToPhp) {
    try {
      postsToPhp = /\/form\.php/.test(await readFile(path.join(outDir, 'contact', 'index.html'), 'utf8'))
    } catch {
 ...
 */
export async function prepareServerFiles(outDir, build, { allowDemo = false, env = process.env } = {}) {
  ...
  await writeFile(path.join(outDir, '.htaccess'), htaccess({ baseUrl: build.baseUrl }), 'utf8')
