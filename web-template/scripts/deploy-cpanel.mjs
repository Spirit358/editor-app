...
class Cpanel {
  ...
  async login() {
    ...
    this.jar = path.join(await mkdtemp(path.join(os.tmpdir(), 'cpanel-')), 'jar')
    const body = await this.curl([
      '-X', 'POST',
      '--data-urlencode', `user=${this.user}`,
      '--data-urlencode', `pass=${this.password}`,
      `${this.base}/login/?login_only=1`,
    ])
    const json = parse(body, 'login')
    if (json.status !== 1 || !json.security_token) {
      fail(`cPanel login failed: ${json.message ?? json.reason ?? body.slice(0, 200)}`)
    }
    this.session = json.security_token
  }

  async close() {
    if (this.jar) await rm(path.dirname(this.jar), { recursive: true, force: true })
  }
  ...
  async uapi(module, func, params = {}, form = null) {
    ...
    if (json.status !== 1) {
      throw new Error(`${module}::${func}: ${(json.errors ?? ['unknown error']).join('; ')}`)
    }
    return json.data
  }
...
async function listing(cp, dir) {
  try {
    const rows = await cp.uapi('Fileman', 'list_files', { dir, show_hidden: 1, include_mime: 0 })
...
async function main() {
  ...
  const chat = await prepareServerFiles(OUT_DIR, build, { allowDemo: has('--allow-demo') })
  const files = await walk(OUT_DIR)
  const bytes = await sizeOf(files)

  const { CPANEL_URL, CPANEL_HOST, CPANEL_USER, CPANEL_TOKEN, CPANEL_PASSWORD } = process.env
  ...
  if (!CPANEL_URL || !CPANEL_USER) fail('Set CPANEL_URL and CPANEL_USER in the environment or .env.local.')
  ...
  try {
    ...
    if (present === null) console.log('  the web root does not exist yet')
      ...
      console.log(`  it holds ${present.length} entr${present.length === 1 ? 'y' : 'ies'}${wordpress ? ' — a WordPress install' : ''}`)
    }

    if (has('--check')) {
      if (present?.length) {
        for (const name of present.slice(0, 40)) console.log(`    ${name}`)
        if (present.length > 40) console.log(`    … and ${present.length - 40} more`)
      }
      console.log('')
      return
    }
    ...
    const upload = await cp.uapi('Fileman', 'upload_files', {}, { dir: home, 'file-1': `@${archive}` })
    ...
    await cp.api2('Fileman', 'fileop', { op: 'extract', sourcefiles: remoteArchive, destfiles: docroot })
    await cp.api2('Fileman', 'fileop', { op: 'unlink', sourcefiles: remoteArchive })
    const after = (await listing(cp, docroot)) ?? []
    if (!after.includes('index.html')) {
      throw new Error(`extract finished but ${docroot} has no index.html (${after.length} entries)`)
    }
