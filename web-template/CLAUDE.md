...
| `activa` | pl-PL | demo built, imagery pending | Szczecinek heating-tech wholesaler/installer (Google: "HVAC contractor", 4.2★/32); live domain shows a blank WordPress default; GBP has no hours and a poor lead photo. Run `pnpm imagery --slug activa` from a machine with API access, then shoot + audit again. `[VERIFY]` items in the config are the intake questions; ask for the logo and the best review texts. |
