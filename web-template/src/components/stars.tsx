...
/** Rating stars drawn as clipped shapes, so fractions are exact. */
export function Stars({
  ...
  return (
    <span className={cn('inline-flex items-center', className)} role="img" aria-label={label ?? `${value}/5`}>
      <span className="relative inline-block" style={{ height: size }}>
        <Row size={size} className="text-rule-strong" />
        <span className="absolute inset-0 overflow-hidden" style={{ width: `${pct}%` }}>
          <Row size={size} className="text-accent" />
        </span>
      </span>
    </span>
  )
